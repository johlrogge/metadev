...
| polylith | sonnet | Polylith architecture expert — design, scaffold, analyse, and migrate Rust/Cargo projects (cargo-polylith 0.12.0) |
...
The polylith agent uses the `cargo-polylith` MCP server (0.12.0), which exposes read tools (`polylith_info`, `polylith_deps`, `polylith_check`, `polylith_status`) and write tools including `polylith_bump` (workspace version bumping), `polylith_change_profile` (regenerating root `Cargo.toml` from a named profile), and `polylith_migrate_package_meta` (moving `[workspace.package]` metadata from `Polylith.toml` to root `Cargo.toml`).
